# IronNestFCS Community Patch 1.2.0
...
> 1.2.0 的重点是把自动扫荡从“固定任务队列”改成“动态候选池 + 真实击发预览 + 方向机行进链 + 弹药状态恢复”的速射调度模型。
...
- **战术雷达**：每 3 秒自动扫描战场，通过实体 Icon/Role
  位掩码精准识别敌方阵营（基于 [Iron Nest DB](https://ironnestdb.com/enemies) 数据），自动排除平民、警察、友军、参照点等非敌对目标。
- **持续扫荡模式**：`Numpad 0` / `Ctrl+0` 切换。开启后新出现的敌对目标会进入候选池，面板显示 `[Sweep ON]`。
- **双重标点模式**：`Numpad 5` / `Ctrl+5` 切换自动/手动标点。自动模式由雷达放置 T1-T4；手动模式保留玩家拖动。
...
| --- | --- | --- |
| `F9` | - | 热重载火控逻辑（开发用） |
| `Numpad 0` | `Ctrl+0` | 切换持续扫荡模式，并清除资源 halt 状态 |
...
方向机是左右炮共享资源。1.2.0 中，方向机调度遵循以下原则：
...
4. 按 **Numpad 5** 切换标点模式：`Auto` 由雷达自动标记，`Manual` 保留玩家手动拖动。
5. 按 **Numpad 0** 开启持续扫荡，面板显示 `[Sweep ON]`。
